...
for i=1 : no_clases
    ...
    for j = 1: no_imagenes
        % Cargar la imagen actual
        imagen = imread(fullfile(direccion, imagenes(j).name));
        mask = segmentImage(imagen);

        stats = regionprops(mask, 'Area');
        total_area = sum([stats.Area]);
        fprintf('Imagen %s: Área total = %.2f píxeles\n', imagenes(j).name, total_area);
        
        % Mostrar imagen original con máscara aplicada (resaltada en rojo)
        figure;
        imshow(imagen);
        hold on;
        h = imshow(cat(3, ones(size(mask)), zeros(size(mask)), zeros(size(mask))));
        set(h, 'AlphaData', 0.3 * mask);  % superponer máscara en rojo con transparencia
        
        title(sprintf('Imagen: %s', imagenes(i).name));
        
        % Esperar que el usuario presione ENTER
        input('Presiona ENTER para continuar...', 's');
        
        close;  % cerrar la figura para limpiar antes de la siguiente
